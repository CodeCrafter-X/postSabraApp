...
export default function ManageNoticePage() {
  ...
  const deleteNotice = async (id) => {
    try {
      ...
      setNotices((prev) => prev.filter((u) => u._id !== id));
      alert("✅ " + data.message);
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto backdrop-blur-md bg-white/80 p-8 rounded-2xl shadow-xl">
        ...
        {loading ? (
          ...
          <div className="text-center p-10 bg-gray-50 rounded-xl shadow">
            <p className="text-gray-600 text-xl">No notices found 📭</p>
          </div>
        ) : (
          <>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100 text-lg">
                  {currentNotices.map((u, idx) => (
                    ...
                    >
                      <td className="px-6 py-4 text-gray-900 font-medium">{u.title}</td>
                      <td className="px-6 py-4 text-gray-700">
                        {u.content.length > 60 ? u.content.slice(0, 60) + "..." : u.content}
                      </td>
                      <td className="px-6 py-4 text-gray-700">{u.category}</td>
                      <td className="px-6 py-4 text-gray-700">{u.postedBy?.username || "Unknown"}</td>
                      <td className="px-6 py-4 text-gray-500">{u.postedBy?.email || "No email"}</td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => deleteNotice(u._id)}
